 ...
 */
import * as go from '../../station_graphics/gojs/release/go';
/**
 ...
 */
export class AvoidsLinksRouter extends go.Link {
  constructor(init) {
    super();
...
class _SegInfo {
  ...
  _computeGeoH() {
    ...
    while (j2 < this.link.pointsCount - 1 && Math.abs(this.link.getPoint(j2).x - this.link.getPoint(j2 + 1).x) < 0.5) {
      j2++;
    }
  ...
  _computeGeoV() {
    ...
    while (j2 < this.link.pointsCount - 1 && Math.abs(this.link.getPoint(j2).y - this.link.getPoint(j2 + 1).y) < 0.5) {
      j2++;
    }
