<template>
  <div class="p-fluid">
    ...
    >
      ...
      <Column field="enabled" header="Status" style="width: 8%">
        <template #editor="{ data, field }">
          <Dropdown
            v-model="data[field]"
            :options="statuses"
      ...
      <Column field="gen_scale" header="Gen scale" style="width: 15%">
        <template #editor="{ data, field }">
          <MultiSelect
            v-model="data[field]"
            :options="listTrips"
        ...
        <template #body="slotProps">
          <span v-for="(item, index) in slotProps.data.gen_scale.slice(0, 3)" :key="index">
            {{ item }}
            <span v-if="index === 2 && slotProps.data.gen_scale.length > 3"> ...</span>
            <span v-else="index < 2 && index === slotProps.data.gen_scale.length - 1">, </span>
          </span>
        </template>
      </Column>
      <Column field="lines" header="Lines" style="width: 15%">
        <template #editor="{ data, field }">
          <MultiSelect
            v-model="data[field]"
      ...
      <Column field="Contingencies_id" header="Contingencies" style="width: 15%">
        <template #editor="{ data, field }">
          <MultiSelect
            v-model="data[field]"
            :options="listContingencies"
