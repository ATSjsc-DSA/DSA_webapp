              ...
              <div v-for="(item, index) in slotProps.items" :key="index" class="col-12">
                <div
                  ...
                  :class="{
                    ...
                    'selected-item': frequencySelected && frequencySelected.name === item.name,
                  }"
                  @click="frequencyClick(item)"
                >
                  <div class="flex flex-row justify-content-between align-items-center gap-2 flex-1 ml-2">
                    ...
                    <Button
                      ...
                      rounded
                      severity="danger"
                      @click="confirmDeleteFrequency($event, item)"
                    />
                  </div>
                </div>
              </div>
...
const frequencyClick = async (item) => {
  frequencySelected.value = item;
  await getFrequencyData();
};

const getFrequencyData = async () => {
  try {
    const res = await ApiFrequency.getFrequencyById(frequencySelected.value._id);
...
const createFrequency = async () => {
  try {
    ...
    await getFrequencyList();
    frequencySelected.value = res.data;
    frequencyData.value = res.data;
...
const confirmDeleteFrequency = (event, frequency) => {
  confirm.require({
    ...
    acceptLabel: 'Delete',
    accept: async () => {
      await deleteFrequency(frequency._id);
    },
    reject: () => {},
  });
};
