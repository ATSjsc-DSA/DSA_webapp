<template>
  <div class="card">
    <FileUpload name="gen" multiple :fileLimit="fileLimit" accept=".xlsx, .xls, .csv">
      <template #header="{ chooseCallback, clearCallback, files }">
        <div class="w-full flex flex-wrap justify-content-between align-items-center flex-1 gap-2">
          <div class="font-semibold text-lg">EMS and Library file upload</div>
          <div>
            <Button
      ...
      <template #content="{ removeFileCallback, uploadCallback, files }">
        ...
        <div v-else class="flex align-items-center justify-content-center flex-column">
          ...
          <p class="mt-4 mb-0">Drag and drop EMS and GEN files to here to upload.</p>
        </div>
        <div v-if="files.length === 1" class="flex align-items-center justify-content-center flex-column">
          <p class="mt-4 mb-0">Drag and drop EMS or GEN files to here to upload.</p>
        </div>
        <div class="mt-6 w-full flex flex-wrap justify-content-end align-items-center flex-1 gap-2">
          <Button
            icon="pi pi-save"
            label="Upload"
            severity="success"
