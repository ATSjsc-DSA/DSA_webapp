...
const { psm_active, projectData, seedConfigVersionId } = storeToRefs(commonStore);
...
export class ApiContingencies {
  static async getListContingencies() {
    return get(`/seedconfig/${projectData.value._id}/${seedConfigVersionId.value}/contingencies`);
  }
  static async searchSubsystem(query = '', data = []) {
    return post(
      `/seedconfig/${projectData.value._id}/${seedConfigVersionId.value}/search/contingencies?query=${query}`,
      data,
    );
  }
  static async createContingencies(data) {
    return post(`/seedconfig/${projectData.value._id}/${seedConfigVersionId.value}/contingencies`, data);
  }
...
export class ApiDisturbances {
  static async getList(tsaId, page) {
    return get(`/seedconfig/${projectData.value._id}/${seedConfigVersionId.value}/${tsaId}/disturbances`, {
      page: page,
      page_size: 10,
    });
  }
  ...
  static async createDisturbances(tsaId, data) {
    return post(`/seedconfig/${projectData.value._id}/${seedConfigVersionId.value}/${tsaId}/disturbances`, data);
  }
...
export class ApiSubsystem {
  static async getListSubsystem(param) {
    return get(`/seedconfig/${projectData.value._id}/${seedConfigVersionId.value}/subsystem`, param);
  }
  static async getListSubsystemOnlyName() {
    return get(`/seedconfig/${projectData.value._id}/${seedConfigVersionId.value}/subsystemonlyname`);
  }

  static async searchSubsystem(query = '', data = []) {
    return post(
      `/seedconfig/${projectData.value._id}/${seedConfigVersionId.value}/search/subsystem?query=${query}`,
      data,
    );
  }
  static async createSubsystem(data) {
    return post(`/seedconfig/${projectData.value._id}/${seedConfigVersionId.value}/subsystem`, data);
  }
